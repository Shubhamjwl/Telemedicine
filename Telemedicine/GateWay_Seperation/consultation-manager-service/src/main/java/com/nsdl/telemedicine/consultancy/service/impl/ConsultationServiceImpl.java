...
public class ConsultationServiceImpl implements ConsultationService {
			...
			if (scribeEntityList != null && !scribeEntityList.isEmpty()) {
				List<ScribeDtls> scribeList = scribeEntityList.parallelStream()
						.map(dtls -> ScribeDtls.builder().scrbEmailID(dtls.getSrdEmail())
								.scrbFullName(dtls.getSrdScrbName())
								.isDefaultScribe(dtls.getIsDefaultScribe())
								.profilePhoto(dtls.getProfilePhoto())
								.scrbMobileNo(dtls.getSrdMobileNo() != null ? dtls.getSrdMobileNo().toString() : "")
								.scrbUserId(dtls.getSrdUserID()).build())
						.collect(Collectors.toList());
				response.setScribeDtls(scribeList);
	...
	public PrescriptionDetailsDTO getDetailsForPrescription(String appointID) throws IOException {
		...
		if (!EmptyCheckUtility.isNullEmpty(appointID)) {
			...
			if (appointmentEntity != null) {
				...
				String chiefComplainList = String.join("\n", getComlainList(consultCcList));
				...
				details.put(ConsultationConstants.CHIEFCOMPLAINTS.getValue(), chiefComplainList);
	...
	public MainResponseDTO<?> saveConsultationDtls(MainRequestDTO<ConsultationDtlDTO> consultationRequest) {
		...
		if (null != consultationRequest.getRequest()) {
			File handwrittenPrescriptionFile = new File(prescriptionPath + pathSeperator + consultationRequest.getRequest().getCtApptId() + ".pdf");
			...
			if((isPrescriptionExist == false && null != consultationRequest.getRequest().getHandwrittenPrescription() 
					&& !consultationRequest.getRequest().getHandwrittenPrescription().isEmpty()) || isPrescriptionExist == true) {
				if(null != consultationRequest.getRequest().getHandwrittenPrescription() && !consultationRequest.getRequest().getHandwrittenPrescription().isEmpty()) {
					//upload handwritten prescription to given location.
					String filePath = prescriptionPath + pathSeperator + consultationRequest.getRequest().getCtApptId() + ".pdf";
					try {
						...
						e1.printStackTrace();
						throw new ConsultationServiceException(ErrorConstants.SOMETHING_WENT_WRONG.getCode(), ErrorConstants.SOMETHING_WENT_WRONG.getMessage());
					}
					String prescriptionFileInfo[] = consultationRequest.getRequest().getHandwrittenPrescription().split(",");
					String prescriptionFileType = prescriptionFileInfo[0].substring(prescriptionFileInfo[0].indexOf("/") + 1, prescriptionFileInfo[0].indexOf(";"));
	...
	public PrescriptionDetailsDTO getPrescriptionDetails(ConsultationDtlDTO consultationRequest) throws IOException {
		...
		if (null != consultationRequest) {
			...
			if (appointmentEntity != null) {
				...
				if(null == consultationDtl.getCtPrescriptionPath() || consultationDtl.getCtPrescriptionPath().isEmpty()) {
					String medicationList = consultationDtl.getCtMedication();
					String chiefComplainList = consultationDtl.getCtChiefComplaints();
					...
					details.put(ConsultationConstants.DRMOBILENO.getValue(), String.valueOf(docDetails.getDmdMobileNo()));
					details.put(ConsultationConstants.DREMAILID.getValue(), null != docDetails.getDmdEmail() ? docDetails.getDmdEmail() : "");
					...
					// if doctors own template is available use that template or use default template.
					PrescriptionTemplateDetails prescriptionTemplateDetails = prescriptionTemplateRepo.checkIfPrescriptionTempAvailable(consultationRequest.getCtDoctorId().toUpperCase());
					if(null != prescriptionTemplateDetails) {
						pdfFilePath = pdfGenerator.generatePrescriptionPDFAsPerDoctorTemplate(details, prescriptionTemplateDetails.getPtdPrescriptionTemplatePath());
					}else {
	...
	@Override
	public ConsultationResponseDTO saveConsultationPatientReports(@Valid MainRequestDTO<PatientDocumentsDtlsDTO<DocumentReportDTO>> request) throws IOException {
		...
		try {
			...
			if(appointmentDtlsEntity!=null)
			{
				for (int i = 0; i <request.getRequest().getDocument().size(); i++) {
					...
					{
						if(request.getRequest().getDocument().get(i).getFiles().length()<Long.parseLong(documentUploadSize))
						{
						...
						createDirectory(ConsultationConstants.PATIENTREPORTTEMPDIRECTORY.getValue() + File.separator + appointmentDtlsEntity.getPatientRegDtlsEntity().getPrdUserId());
						...
						logger.info("get path of file and save file to particular path");
							temppath = byteStorePath + File.separator + ConsultationConstants.PATIENTREPORTTEMPDIRECTORY.getValue() + File.separator + appointmentDtlsEntity.getPatientRegDtlsEntity().getPrdUserId();
						    ...
						    if(path!=null)
							{
								...
								patientReportUploadDtlsEntity=patientReportUploadDtlsRepository.save(patientReportUploadDtlsEntity);
							}
		...
		if (null != consultationRequest) {
			...
			if (null !=consultationDtls) {	
				try {
					...
					consultationDtlsUpdated.setCtPrescriptionPath(consultationRequest.getRequest().getCtPrescriptionPath());
...
public  void createDirectory(String folder_name) throws IOException {
	...
	public MainResponseDTO<?> uploadPrescriptionTemplate(MainRequestDTO<ConsultationDtlDTO> prescriptionUploadRequest) {
		MainResponseDTO mainResponseDto = new MainResponseDTO();
		PrescriptionTemplateDetails prescriptionTemplateDetail = prescriptionTemplateRepo.checkIfPrescriptionTempAvailable(prescriptionUploadRequest.getRequest().getCtDoctorId().toUpperCase());
		if(null == prescriptionTemplateDetail) //if template is previously not added then only add.
		{
			String templatePath = prescriptionPath + pathSeperator + "templates" + pathSeperator + prescriptionUploadRequest.getRequest().getCtDoctorId() + pathSeperator;
			if(null != prescriptionUploadRequest.getRequest().getTemplateHeader() && !prescriptionUploadRequest.getRequest().getTemplateHeader().isEmpty())
			{
			...
			if(null != prescriptionUploadRequest.getRequest().getTemplateFooter() && !prescriptionUploadRequest.getRequest().getTemplateFooter().isEmpty())
			{
	...
	/**
	 * Added by girishk to upload doctors own prescription template to specified location.
	 * This is used if doctor want to generate prescription as per his own template.
	 */
		...
		if(null!=patientReportUploadDtlsEntityList && patientReportUploadDtlsEntityList.size()>0)
		{
			...
			try 
			{
				for (int i = 0; i < patientReportUploadDtlsEntityList.size(); i++) {
					...
					try{
						finalResponse = new String(Base64.encodeBase64(Files.readAllBytes(Paths.get(patientReportUploadDtlsEntityList.get(i).getPath()))));
					...
					response.setMimetype(URLConnection.guessContentTypeFromName(patientReportUploadDtlsEntityList.get(i).getReportName()));
					list.add(response);
				}
	...
	private void saveConsultationDetails(MainRequestDTO<ConsultationDtlDTO> consultationRequest, String prescriptionPath) {
		...
		try {
			...
			if (null != consultationDtls) {
				...
				consultationDtlsEntity
						.setCtChiefComplaints(consultationRequest.getRequest().getCtChiefComplaints());
	...
	private void uploadHandwrittenPrescription(String filePath, String fileType, byte[] data) {
		...
		if(fileType.equalsIgnoreCase("pdf")) {
		    ...
		    try (OutputStream outputStream = new BufferedOutputStream(new FileOutputStream(file))) {
		        ...
		        e.printStackTrace();
		        throw new ConsultationServiceException(ErrorConstants.ERROR_UPLOADING_HANDWRITTEN_PRESCRIPTION.getCode(),
	...
	private void sendPrescriptionOnEmail(TemplateDtls details){
		...
		try {
			...
			ResponseEntity<MainResponseDTO<OtpResponseDTO>> response = template.exchange(prescriptionEmailURL,HttpMethod.POST, requestEntity, parameterizedResponse);
		}catch(Exception e) {
