...
public interface RegistrationRepo extends JpaRepository<DoctorRegDtlsEntity, String> {
	...
	@Query(value = "select * from registration.dr_reg_dtls where drd_user_id in :doctertUserID", nativeQuery = true)
	public List<DoctorRegDtlsEntity> findDoctorDetailsByUserIDs(@Param("doctertUserID") List<String> doctertUserID);
	...
	@Query(value = "select * from registration.dr_reg_dtls where drd_verified_lvl2_tmstmp >=:fromDate and drd_verified_lvl2_tmstmp <=:toDate and drd_specialiazation=:specialiazation and drd_city=:city and drd_association_name=:associationName ", nativeQuery = true)
	public List<DoctorRegDtlsEntity> findRegDateBetween(@Param("fromDate") LocalDateTime startTime,@Param("toDate") LocalDateTime endTime,@Param("specialiazation") String specialiazation,@Param("city") String city,@Param("associationName") String associationName );
