...
public interface DoctorSessionRepo extends JpaRepository<DoctorSessionHistory, String>{
	...
	public void updateLogOutTime(@Param("userId") String userId,@Param("logOutTime") LocalDateTime logOutTime);
	  ...
	  @Query(value ="SELECT * FROM usrmgmt.session_history where session_history.sh_created_tmpstmp = ?1 and sh_usr_id= ?2", nativeQuery = true) 
	public DoctorSessionHistory getCurrSessionDtls(LocalDateTime currTime,String userId);
	...
	public DoctorSessionHistory getSessionId(@Param("userId") String userId,@Param("flag") String flag);
	 ...
	 @Query(value ="SELECT * FROM usrmgmt.session_history where session_history.sh_created_tmpstmp >=:startTime and sh_end_tmpstmp <=:endTime", nativeQuery = true) 
	 public List<DoctorSessionHistory> getSessionHistory(@Param("startTime") LocalDateTime startTime,@Param("endTime") LocalDateTime endTime);
