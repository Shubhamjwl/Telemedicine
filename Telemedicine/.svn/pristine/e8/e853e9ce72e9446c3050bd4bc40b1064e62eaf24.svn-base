...
public interface AppointmentDtlsRepository extends JpaRepository<AppointmentDtlsEntity, Long> {
	...
	@Query(value = "SELECT * FROM appointment.appt_dtls where ad_appt_date_fk in (?1, ?2) and ad_appt_status in ('S', 'R', 'C') and ad_dr_user_id_fk = ?3 order by ad_appt_date_fk, ad_appt_slot_fk", nativeQuery = true)
	...
	@Query(value = "SELECT * FROM appointment.appt_dtls where appt_dtls.ad_pt_user_id_fk = ?1 and ad_appt_status in ('S','R') and ad_appt_date_fk>=current_date order by ad_appt_date_fk, ad_appt_slot_fk", nativeQuery = true)
	...
	@Query(value = "SELECT * FROM appointment.appt_dtls where ad_appt_date_fk in (?1) and ad_appt_status in ('S', 'R', 'C') and ad_dr_user_id_fk = ?2 order by ad_appt_date_fk, ad_appt_slot_fk", nativeQuery = true)
