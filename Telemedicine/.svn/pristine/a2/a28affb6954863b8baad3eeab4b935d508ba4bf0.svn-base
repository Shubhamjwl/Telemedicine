...
public class DoctorVerificationController {
	...
	public ResponseEntity<MainResponseDTO<CommonResponseDTO>> updateLinkForExistingPatient(
			@RequestBody @Valid MainRequestDTO<UpdateLinkDTO<GetDoctorDetailsRequestDTO>> request) {
	...
	@PostMapping(value = "/updateLinkForExistingDoctor", produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<MainResponseDTO<CommonResponseDTO>> updateLinkForExistingDoctor(
			@RequestBody @Valid MainRequestDTO<UpdateLinkDTO<GetDoctorDetailsRequestDTO>> request) {

		MainResponseDTO<CommonResponseDTO> responseWrapper = (MainResponseDTO<CommonResponseDTO>) AuthUtil
				.getMainResponseDto(request);
		try {
			responseWrapper.setResponse(doctorVerificationService.updateLinkForDoctor(request.getRequest()));
		} catch (Exception e) {
			e.printStackTrace();
			return ResponseEntity.status(HttpStatus.EXPECTATION_FAILED).body(responseWrapper);
		}
	...
	@SuppressWarnings("unchecked")
	@PostMapping(value = "/allowDoctorsForLogin", produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<MainResponseDTO<String>> updateLoginAllowedFlagForDoctor(
			@Valid @RequestBody MainRequestDTO<List<String>> request) {
		logger.info("Request Received to update user login allowed flag for doctor id.");

		MainResponseDTO<String> response = new MainResponseDTO<>();

		response = (MainResponseDTO<String>) AuthUtil.getMainResponseDto(request);
		response.setResponse(doctorVerificationService.updateLoginAllowedFlagForDoctor(request.getRequest()));
		response.setResponsetime(LocalDateTime.now());
		response.setStatus(true);
		return ResponseEntity.status(HttpStatus.OK).body(response);
	}
	
	@GetMapping(value = "/getListOfDoctorsToAllowLogin", produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<MainResponseDTO<List<DoctorRegDtlsDTO>>> getListOfDoctorToAllowLogin() {
		logger.info("Request Received to get doctors for which login allowed flag is false.");

		MainResponseDTO<List<DoctorRegDtlsDTO>> response = new MainResponseDTO<>();
		response.setResponse(doctorVerificationService.getListOfDoctorToAllowLogin());
		response.setResponsetime(LocalDateTime.now());
		response.setStatus(true);
		return ResponseEntity.status(HttpStatus.OK).body(response);
	}
