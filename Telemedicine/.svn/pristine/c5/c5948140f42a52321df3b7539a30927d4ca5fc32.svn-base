...
public class DoctorDeRegServiceImpl implements DoctorDeRegService {
	...
	private MainResponseDTO<OtpResponseDTO> generateOTPDoctorUser(DoctorRegDtlsEntity docRegEntity) {
		...
		ResponseEntity<MainResponseDTO<OtpResponseDTO>> response = restTemplate.exchange(generateOtpURL,
				HttpMethod.POST, requestEntity, parameterizedResponse);
		logger.info("OTP manager REST-call sucess with desc:" + response.getBody().getResponse().getDescription()
				+ "and message:" + response.getBody().getResponse().getMessage());
