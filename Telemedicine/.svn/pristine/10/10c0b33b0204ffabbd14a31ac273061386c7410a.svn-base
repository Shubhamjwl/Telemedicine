...
export class PatientRegisteredComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.route.queryParams.subscribe((params) => {
      ...
      if (params && params.stateCode) {
        this.stateCode = params.stateCode;
      }
      if (params && params.districtCode) {
  ...
  submit() {
    ...
    this.healthIDDTO.gender = this.selectedOption.substring(0, 1);
    ...
    console.log('healthIDDTO', this.healthIDDTO);
    this._servises
      ...
      .subscribe(
        (result: any) => {
          ...
          this.healthId = result.response.healthId.substring(
            0,
            result.response.healthId.length - 4
          );
