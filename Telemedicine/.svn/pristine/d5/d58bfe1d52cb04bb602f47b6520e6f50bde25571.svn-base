...
public class OtpServiceImpl implements OtpService {
	...
	public MainResponseDTO<OTPResponse> generateOTP(MainRequestDTO<OTPRequest> request) {
		...
		if (Utility.stringIsNullOrEmpty(payLoad.getMobileNo())) {
			...
			throw new OTPException(new ServiceErrors(ErrorConstant.INVALID_MOBILE.getCode(),
					ErrorConstant.INVALID_MOBILE.getMessage()));
		}
...
//			}
		...
		if ((Utility.stringIsNullOrEmpty(payLoad.getOtpFor()))
				|| (!payLoad.getOtpFor().equalsIgnoreCase(AppConstant.REG)
						&& !payLoad.getOtpFor().equalsIgnoreCase(AppConstant.FORGOT)
						&& !payLoad.getOtpFor().equalsIgnoreCase(AppConstant.CHANGE)
						&& !payLoad.getOtpFor().equalsIgnoreCase(AppConstant.DE_REG)
						&& !payLoad.getOtpFor().equalsIgnoreCase(AppConstant.LINK_RECORD))) {
			logger.info(
					"Invalid Value found for parameter otpFor. Value must be any of the following reg/forgot/change/Deregister , value found:"
							+ payLoad.getOtpFor());
			throw new OTPException(new ServiceErrors(ErrorConstant.INVALID_PARAMETER.getCode(),
					ErrorConstant.INVALID_PARAMETER.getMessage()));
		}
		if ((Utility.stringIsNullOrEmpty(payLoad.getOtpGenerateTpye()))
				|| (!payLoad.getOtpGenerateTpye().equalsIgnoreCase(AppConstant.SAME)
						&& !payLoad.getOtpGenerateTpye().equalsIgnoreCase(AppConstant.DIFFERENT))) {
			logger.info(
					"Invalid Value found for parameter otpGenerateTpye. Value must be any of the following same/Different , value found:"
							+ payLoad.getOtpGenerateTpye());
			throw new OTPException(new ServiceErrors(ErrorConstant.INVALID_PARAMETER.getCode(),
					ErrorConstant.INVALID_PARAMETER.getMessage()));
		}
		if ((Utility.stringIsNullOrEmpty(payLoad.getSendType()))
				|| (!payLoad.getSendType().equalsIgnoreCase(AppConstant.SMS)
						&& !payLoad.getSendType().equalsIgnoreCase(AppConstant.EMAIL)
						&& !payLoad.getSendType().equalsIgnoreCase(AppConstant.BOTH))) {
			logger.info(
					"Invalid Value found for parameter sendType. Value must be any of the following Email/SMS/Both , value found:"
							+ payLoad.getSendType());
			throw new OTPException(new ServiceErrors(ErrorConstant.INVALID_PARAMETER.getCode(),
					ErrorConstant.INVALID_PARAMETER.getMessage()));
		}
		...
		if (payLoad.getSendType().equalsIgnoreCase(AppConstant.SMS)) {
			...
			// emailService.sendMail(payLoad.getEmailID(), payLoad.getUserId(), emailOtp,
			// payLoad.getOtpFor());
			payLoad.setSmsOtp(smsOtp);
			sendSMS(payLoad);
			payLoad.setEmailOtp(emailOtp);
	...
	private void sendSMS(OTPRequest payLoad) {
		...
		logger.info("templateTypeName {}" ,templateTypeName);
		logger.info("templateTypeName dtls {}" ,dtls);
	...
	private boolean verifyOTPonMobile(VerifyOTPRequest payLoad) {
		...
		if (userLitInDesc.size() > 0) {
			...
			if (usrOtpEmailVerifyDtl.getOtpType().equalsIgnoreCase(AppConstant.BOTH)
					&& (Utility.stringIsNullOrEmpty(payLoad.getEmailOTP()))) {
			...
			if (!isOTPExpire) {
				...
				if (isPwdValid) {
					...
					if (retryCount > otpVerificationAttempt) {
						...
						throw new OTPException(new ServiceErrors(ErrorConstant.MAX_ATTEMPT_EXCEED.getCode(),
								ErrorConstant.MAX_ATTEMPT_EXCEED.getMessage()));
					}
			...
			throw new OTPException(
					new ServiceErrors(ErrorConstant.USR_NOT_EXIST.getCode(), ErrorConstant.USR_NOT_EXIST.getMessage()));
		}
	}
		...
		if (userLitInDesc.size() > 0) {
			...
			if (!isOTPExpire) {
				...
				if (isPwdValid) {
					...
					repo.save(usrOtpEmailVerifyDtl);
					logger.info(
							"OTP Verififid Successfully. Reocrd updated successfully into UsrOtpEmailVerifyDtl table");
					return true;
				} else {
					retryCount += 1;
					if (retryCount > otpVerificationAttempt) {
...
//Method to send invitation link to mobile/email
	@SuppressWarnings("unchecked")
	@Override
	public MainResponseDTO<InvitationLinkResponse> sendInvitationLink(
			MainRequestDTO<InvitationLinkRequest> invitationLinkRequest) {
		...
		if (Utility.stringIsNullOrEmpty(request.getInviteMode())) {
			...
			throw new OTPException(new ServiceErrors(ErrorConstant.INVALID_INVITE_MODE.getCode(),
					ErrorConstant.INVALID_INVITE_MODE.getMessage()));
		}
		if (Utility.stringIsNullOrEmpty(request.getContent())) {
			logger.info("Content is blank or null ");
			throw new OTPException(new ServiceErrors(ErrorConstant.INVALID_CONTENT.getCode(),
					ErrorConstant.INVALID_CONTENT.getMessage()));
		}
		if (null != request.getInviteOn() && !request.getInviteOn().isEmpty()
				&& request.getInviteMode().equalsIgnoreCase(AppConstant.EMAIL))// condition changed by girishk
		{
