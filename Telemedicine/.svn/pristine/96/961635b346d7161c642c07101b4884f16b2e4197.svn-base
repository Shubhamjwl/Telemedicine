...
import com.nsdl.payment.repository.WalletRepo;
...
public class PaymentServiceImpl implements PaymentService {
	...
	public MainResponseDTO<CommonResponseDTO> updateOrder(UpdateRequestDto paymentRequest) {
		...
		if(paymentRequest.getTotalWalletAmount() > 0) {
		PatientPersonalDetailEntity patientEntity = registrationRepo.findByPtUserIDIgnoreCase(loggedInUser);
		if (patientEntity != null) {
			if(!patientEntity.getWalletAmount().equals(paymentRequest.getTotalWalletAmount()) && patientEntity.getWalletAmount() != paymentRequest.getTotalWalletAmount()) {
				throw new DateParsingException(new ServiceError(ErrorConstants.INVALID_REQUEST.getCode(),
						ErrorConstants.INVALID_REQUEST.getMessage()));
			}
		}
	...
	public MainResponseDTO<CommonResponseDTO> updatePaymentResponse(RazorPayResponse paymentResponse) {
		...
		if (payRepo.saveAndFlush(paymentDtl) != null) {
			...
			if(paymentResponse.getUsedWalletAmount() > 0) {
			WalletEntity walletEntity = new WalletEntity();
			walletEntity.setWalPatUserId(paymentResponse.getLoggedinUser());
			walletEntity.setWalTranstAmount(paymentResponse.getUsedWalletAmount());
			walletEntity.setWalTranstTmpstmp(LocalDateTime.now());
			walletEntity.setWalTranstType(AppConstant.DEBIT);
			walletEntity.setWalTranstBy(loggedInUser);
			walletRepo.saveAndFlush(walletEntity);
		    
		    PatientPersonalDetailEntity patientEntity= registrationRepo.findByPtUserIDIgnoreCase(loggedInUser);
		    patientEntity.setWalletAmount(paymentResponse.getTotalWalletAmount());
		    registrationRepo.saveAndFlush(patientEntity);
			}
	...
	public MainResponseDTO<CommonResponseDTO> updatePaymentLinkDtls(RazorPayResponse paymentResponse) {
		...
		if(stringIsNullOrEmpty(paymentResponse.getTransId()))
		{
			...
			throw new DateParsingException(new ServiceError(ErrorConstants.INVALID_REQUEST.getCode(), ErrorConstants.INVALID_REQUEST.getMessage()));
		}
	...
	/*	PatientPersonalDetailEntity patientDtl=registrationRepo.findByPtUserIDIgnoreCase(loggedInUser);
		if(null!=patientDtl)
		{
			...
			logger.info("Trying to update record against transactionId: "+paymentResponse.getTransId());
			 ...
			 if (payRepo.saveAndFlush(paymentDtl) != null) 
			 {
				 logger.info("Record updated successfully in payment details against tarnsactionId: ",paymentResponse.getOrderId());
