...
public class PaymentServiceImpl implements PaymentService {
	...
	public MainResponseDTO<CommonResponseDTO> updatePaymentResponse(RazorPayResponse attributes) {
		...
		try {
			...
			logger.info("Request received for order id...." + attributes.getOrderId() + " and payment id "
					+ attributes.getPaymentId() + "and Signature is" + attributes.getSignature());
			logger.info("Request received for usedWalletAmount" + attributes.getUsedWalletAmount() + " and totalWalletAmount "
					+ attributes.getTotalWalletAmount());
			// 1 Verify Signature
			...
			if (null != payment) {
				...
				int walletBalance = attributes.getTotalWalletAmount() - attributes.getUsedWalletAmount();
				attributes.setTotalWalletAmount(walletBalance);
				attributes.setUsedWalletAmount(attributes.getUsedWalletAmount());
	...
	public MainResponseDTO<PaymentResponse> createOrder(PayRequest paymentRequest) {
		...
		int totalCharges = calculateTotalCharges(paymentRequest);
		try {
			RazorpayClient razorPayclient = new RazorpayClient(razorPayKeyId, razorPayKeySecret);
			...
			razorPay = fetchRazorPay((String) order.get("id"),totalCharges);
			...
			updateDto.setAmount(totalCharges);
			...
			updateDto.setTotalWalletAmount(paymentRequest.getTotalWalletAmount());
			try {
	...
	private int calculateTotalCharges(PayRequest paymentRequest) {
		if(paymentRequest.getUsedWalletAmount() > paymentRequest.getConsultationFee()) {
			throw new DateParsingException(new ServiceError(ErrorConstants.INVALID_CONSULT_AMOUNT.getCode(),
					ErrorConstants.INVALID_CONSULT_AMOUNT.getMessage()));
		}	
	
		if(paymentRequest.getTotalWalletAmount() < paymentRequest.getUsedWalletAmount()) {	
			throw new DateParsingException(new ServiceError(ErrorConstants.INVALID_WALLET_AMOUNT.getCode(),
					ErrorConstants.INVALID_WALLET_AMOUNT.getMessage()));
		}
		
		int paidAmount=paymentRequest.getConsultationFee() - paymentRequest.getUsedWalletAmount();
		int finalAmount=paidAmount + paymentRequest.getConvenienceCharge();	
		return finalAmount;
	}
	...
	public RazorPay fetchRazorPay(String orderId,int totalCharges) {
		RazorPay razorPay = new RazorPay();
		razorPay.setApplicationFee(convertRupeeToPaise(String.valueOf(totalCharges)));
