...
public class HealthIdCreationByAdharServiceImpl implements HealthIdCreationByAdharService {
	...
	public MainResponseDTO<GenerateOtpResp> generateOTPForAadhar(Map<String, String> headers,
		...
		if (generateOtpAadhaarDTO != null && generateOtpAadhaarDTO.getAadhaar() != null) {
			try {
				uri = new URI(url);
			...
			resp.setErrors(CustomErrorUtils.throwExceptionForAadhaar());
		}
		...
		if (resendOTPDTO != null && resendOTPDTO.getTxnId() != null) {
			try {
				...
				HttpEntity<String> requestEntity = new HttpEntity<>(encryptedString,
						commonHeadersUtil.getHeaders(headers));
				result = restTemplate.exchange(uri, HttpMethod.POST, requestEntity, String.class);
				respData = new Gson().fromJson(result.getBody(), AadharOtpResendRespDTO.class); // Json to response
																								// mapping
	...
	public MainResponseDTO<ConfirmOtpRespAadhaar> confirmOTP(Map<String, String> headers,
		...
		if (confirmOtpAadhaarDTO != null && confirmOtpAadhaarDTO.getTxnId() != null) {
			if (validate.validateOtp(confirmOtpAadhaarDTO.getOtp())) {
				try {
					...
					HttpEntity<String> requestEntity = new HttpEntity<>(encryptedString,
							commonHeadersUtil.getHeaders(headers));
					result = restTemplate.exchange(uri, HttpMethod.POST, requestEntity, String.class);
					respData = new Gson().fromJson(result.getBody(), ConfirmOtpRespAadhaar.class); // Json to response
																									// mapping
					resp.setResponse(respData);
					resp.setStatus(HealthIdCreationConstands.SERVICE_SUCCESS_STATUS.isStatus());
				} catch (HttpClientErrorException | HttpServerErrorException e) {
	...
	public MainResponseDTO<GenerateOtpResp> generateMobileOTP(Map<String, String> headers,
		...
		MainResponseDTO<GenerateOtpResp> resp = new MainResponseDTO<>();
		GenerateOtpResp respData = null;
		if (generateMobileOTP != null && generateMobileOTP.getMobile() != null
				&& validate.validateMobileNo(generateMobileOTP.getMobile())) {
			try {
				uri = new URI(url);
	...
	public MainResponseDTO<ConfirmOtpRespAadhaar> verifyMobileOTP(Map<String, String> headers,
		...
		MainResponseDTO<ConfirmOtpRespAadhaar> resp = new MainResponseDTO<>();
		ConfirmOtpRespAadhaar respData = null;
		if (confirmOtpDTO != null && confirmOtpDTO.getTxnId() != null) {
			if (validate.validateOtp(confirmOtpDTO.getOtp())) {
				try {
	...
	/*
	 * creation of healthid with preverified mobile no and aadhar no Demo
	 */
	@Override
	public MainResponseDTO<CreateHealthIdPreverifiedRespDTO> helathIDcreationForPreVerifiedDemo(
		...
		MainResponseDTO<CreateHealthIdPreverifiedRespDTO> responseUsrCreate = createHealthIdPreVerified(headers,
