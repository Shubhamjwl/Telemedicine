...
public interface PatientPersonalDetailsRepository extends JpaRepository<PatientPersonalDetailEntity, Integer>{
	...
	@Query(value = "SELECT * FROM registration.pt_reg_dtls where prd_health_number =:healthId OR prd_health_id=:healthId" , nativeQuery = true)
	PatientPersonalDetailEntity getPatientDtlsById(@Param("healthId") String healthId);
