...
@Builder
@Component
public class OPConsultResourceTwo {
	private static final Logger logger = LoggerFactory.getLogger(OPConsultResourceTwo.class);
	...
	public DocumentReference populateDocumentReferenceResource() {
		...
		documentReferenceDTO.getReportDocList().forEach(reportDoc -> documentReference.getContent()
				.add(new DocumentReferenceContentComponent(new Attachment().setContentType(reportDoc.getDocContentType())
						.setLanguage(reportDoc.getDocLanguage()).setTitle(reportDoc.getDocTitle())
						.setCreationElement(new DateTimeType(reportDoc.getDocCreationDate()))
						.setDataElement(new Base64BinaryType(reportDoc.getDocData())))));
