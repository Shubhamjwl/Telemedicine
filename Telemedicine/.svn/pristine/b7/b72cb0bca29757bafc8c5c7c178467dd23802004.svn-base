	...
	public RouteLocator gatewayRoutes(RouteLocatorBuilder builder) {
		return builder.routes().route(r -> r.path("/login/**")  //usermanagement service URL added by sayali
				...
				.route(r -> r.path("/videoconference/**").filters(
						...
						.uri("lb://VIDEO_CALL_CONFEERENCE-SERVICE/").id("video-call-conference-service"))
				//added by swatis end 
		...
		return (ServerWebExchange ctx, WebFilterChain chain) -> {
			...
			if (CorsUtils.isCorsRequest(request)) {
				...
				headers.add("Access-Control-Allow-Headers",
						"Date, Content-Type, Accept, X-Requested-With, Authorization, From, X-Auth-Token, Request-Id,sessionId,captchaValue,flagValue");
