...
public interface ScribeRegRepo extends JpaRepository<ScribeRegEntity, String>{
	...
	@Query(value = "FROM ScribeRegEntity where scrbUserID =:srdUserId", nativeQuery = false)
	public ScribeRegEntity findScribeDetails(@Param("srdUserId") String srdUserId);
