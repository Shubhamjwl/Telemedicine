...
export class TmRegisterDoctorComponent implements OnInit {
  ...
  registerDoctor() {
    ...
    this.userManagementService.registerDoctor(payload).subscribe((result) => {
      this.spinner.hide();
  ...
  showSelectedFiles(event) {
    ...
    let isFile = fileTypes.some(el => fileName?.includes(el));
    let hasSize = files[0]?.size <= 2000000 ? true : false; //1000000
    if (isFile) {
      if (hasSize && files?.length) {
          for (let file of files) {
            this.uploadedFileNames.push(file);
            const reader = new FileReader();
            reader.onload = (e) => {
              let file = JSON.stringify(e.target.result);
              let name = `:${files[0]?.name}/${this.secondForm.get('documentType').value};`
