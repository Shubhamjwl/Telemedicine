...
public class HealthIdCreationByMobileServiceImpl implements HealthIdCreationByMobileService {
	...
	public MainResponseDTO<GenerateOtpResp> generateOTP(Map<String, String> headers, GenerateOtpDTO generateOtpDTO) {
		logger.info("generateOTP Starts Request for generate OTP");
		...
		if (generateOtpDTO != null && generateOtpDTO.getMobile() != null
				&& validate.validateMobileNo(generateOtpDTO.getMobile())) {

			try {
				uri = new URI(url);
				String encryptedString = new Gson().toJson(generateOtpDTO);
				HttpEntity<String> requestEntity = new HttpEntity<>(encryptedString,
						commonHeadersUtil.getHeaders(headers));
			...
			logger.error(" mobileNo is Empty ");
			resp.setErrors(CustomErrorUtils.throwExceptionForHealthIdMobileNo());
		}

		logger.info("generateOTP Request For Generate OTP Ends");
		return resp;
	}
	...
	public MainResponseDTO<ResendOtpResp> resendOTP(Map<String, String> headers, ResendOtpDTO resendOtpDTO) {
		...
		if (resendOtpDTO != null && resendOtpDTO.getTxnId() != null) {
			try {
				uri = new URI(url);
			...
			resp.setErrors(CustomErrorUtils.throwExceptionForTxnId());
		}
	...
	public MainResponseDTO<ConfirmOtpResp> confirmOTP(Map<String, String> headers, ConfirmOtpDTO confirmOtpDTOReq) {
		logger.info("confirmOTP Request Starts for confirm OTP");
		String url = verifyOTPForMobile;
		URI uri;
		ResponseEntity<String> result = null;
		MainResponseDTO<ConfirmOtpResp> resp = new MainResponseDTO<>();
		ConfirmOtpResp respData = null;
		if (confirmOtpDTOReq != null && confirmOtpDTOReq.getTxnId() != null) {
			if (validate.validateOtp(confirmOtpDTOReq.getOtp())) {
				try {
				...
				resp.setErrors(CustomErrorUtils.throwExceptionForOtp());
			}
		...
		logger.info("confirmOTP Request Ends ");
		return resp;
	}
	...
	public MainResponseDTO<HealthIDResp> saveHealthIdByMobile(Map<String, String> headers,
		...
		logger.info("saveHealthIdByMobile method Starts For Creating HealthId By Mobile ");
		...
		if (responseUsrCreate != null && responseUsrCreate.isStatus()) {
			try {
		...
		logger.info("saveHealthIdByMobile method Ends For Creating HealthId By Mobile ");
		return retResponse;
	}
	...
	private ResponseEntity<?> sendHealthIdDetailsToTelemedicine(Map<String, String> headers, HealthIDResp healthIDDTO,
		...
		try {
			if (healthIDDTO.getHealthId() != null) {
				...
				if (healthIDDTO.getHealthId().contains("@sbx")) {
	...
	private MainResponseDTO<HealthIDResp> createHealthId(Map<String, String> headers, HealthIDDTO healthIDDTO) {
		...
		logger.info("createHealthId method starts For Create HealthId Api call");
		...
		try {
			...
			try {
				if (e.getStatusCode() == HttpStatus.BAD_REQUEST
						|| e.getStatusCode() == HttpStatus.UNPROCESSABLE_ENTITY) {
					UnprocessableErrorDTO errorResult = mapper.readValue(responseString, UnprocessableErrorDTO.class);
					error.setReason(errorResult.getDetails().toString());
				}
		...
		logger.info("createHealthId method Ends For Create HealthId Api call");
		return response;
	...
	public MainResponseDTO<HealthIDResp> saveHealthIdDetailsDemo(Map<String, String> headers,
			MainRequestDTO<HealthIDDTO> requestHealthIDDTO) {
		logger.info("saveHealthIdDetailsDemo Method Starts");

		HealthIDDTO healthIDDTO = requestHealthIDDTO.getRequest();
		MainResponseDTO<HealthIDResp> responseUsrCreate = createHealthId(headers, healthIDDTO);

		logger.info("saveHealthIdDetailsDemo Method Ends");
		return responseUsrCreate;
	}
}
