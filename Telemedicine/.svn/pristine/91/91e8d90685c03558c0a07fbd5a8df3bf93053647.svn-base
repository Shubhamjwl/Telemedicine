...
import com.nsdl.telemedicine.patient.dto.NdhmFeatureFlagResponse;
...
public class PatientRegistrationController {
	...
	@Value("${ndhm.feature.flag}")
	private boolean ndhmFeatureFlag;

	@PostMapping(path = "/registration", consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<ResponseWrapper<PatientResponseDto>> savePatientDetails(
			@Valid @RequestBody RequestWrapper<PatientRegistrationDto> patientRegistrationDto) {
		logger.info("Registration-Patient Request Received");
		return ResponseEntity.status(HttpStatus.OK)
				.body(patientRegistrationService.savePatientDetails(patientRegistrationDto));
	}
	 ...
	 * @param res
	 * @return Added by girishk to search completed appointment using different
	 *         search criteria.
	 */
	@PostMapping(value = "/searchCompletedAppointmentsForPatient", produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<ResponseWrapper<List<AppointmentDetailsResponseDTO>>> searchCompletedAppointmentsForPatient(
			@RequestBody RequestWrapper<AppontmentDetailsRequestDTO> appontmentDetailsRequest,
			HttpServletResponse res) {
		logger.info("Request Received to search completed appointments for patient.");
		return ResponseEntity.status(HttpStatus.OK)
				.body(patientRegistrationService.searchCompletedAppointmentsForPatient(appontmentDetailsRequest));
	}

	/**
	 * @param appontmentDetailsRequest
	 * @param res
	 * @return Added by SayaliA to search completed appointment of latest 10.
	 */
	...
	public ResponseEntity<ResponseWrapper<List<AppointmentDetailsResponseDTO>>> listOfCompletedAppointmentsForPatient() {
		logger.info("Request Received to search completed appointments for patient.");
		return ResponseEntity.status(HttpStatus.OK)
				.body(patientRegistrationService.listOfCompletedAppointmentsForPatient());
	}

	/**
	 * @param bulkRegistrationRequest
	 * @return Method added by girishk to register patients in bulk.
	 * @throws Exception
	 */
	@PostMapping(value = "/bulkPatientRegistration", produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<ResponseWrapper<?>> bulkPatientRegistration(
			@RequestBody RequestWrapper<BulkPatientRegistrationDTO> bulkRegistrationRequest) throws Exception {
		logger.info("Request Received for bulk registration of patients.");
		return ResponseEntity.status(HttpStatus.OK)
				.body(patientRegistrationService.bulkPatientRegistration(bulkRegistrationRequest));
	}

	/**
	 * @param appontmentDetailsRequest
	 * @param res
	 * @return Added by SayaliA to register patient by doctor or scribe
	 */
	...
	public ResponseEntity<ResponseWrapper<PatientResponseDto>> saveRegistrationDetails(
			@Valid @RequestBody RequestWrapper<PatientDetailsDTO> patientDetailsDTO) {
		logger.info("Registration of Patient Request by Doctor or Scribe Received");
		return ResponseEntity.status(HttpStatus.OK)
				.body(patientRegistrationService.saveRegistrationDetails(patientDetailsDTO));
	}

	/**
	 * Added by SayaliA to register patient by doctor on link shared by doctor for
	 * single calender(no token required ,patient will get register automatically by
	 * backend )
	 */
	...
	public ResponseEntity<ResponseWrapper<PatientResponseDto>> savePatientRegistrationDetails(
			@Valid @RequestBody RequestWrapper<PatientDetailsDTO> patientDetailsDTO) {
		logger.info("Registration of Patient with link shared by doctor for single calender");
		return ResponseEntity.status(HttpStatus.OK)
				.body(patientRegistrationService.savePatientRegistrationDetails(patientDetailsDTO));
	}

	/**
	 * Added by SayaliA to register patient by doctor on link shared by doctor(no
	 * token required ,patient will get register automatically by backend )
	 */
	@PostMapping(path = "/registerPatientLink", produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<ResponseWrapper<PatientResponseDto>> savePatientRegDetailsByLink(
			@Valid @RequestBody RequestWrapper<TokenDetailsDTO> patientDetailsDTO) {
		logger.info("Registration of Patient with link shared by doctor");
		return ResponseEntity.status(HttpStatus.OK)
				.body(patientRegistrationService.savePatientRegistrationLinkDetails(patientDetailsDTO));
	}
	...
	public ResponseEntity<ResponseWrapper<PatientResponseDto>> updateHealthIDDetails(
			@Valid @RequestBody RequestWrapper<HealthIdDetailsToTelemedicineDTO> healthIdDetailsDTO) {
		logger.info("Receiving healthID Details on telemedicine");
		System.out.println("healthIdDetailsDTO " + healthIdDetailsDTO);
		return ResponseEntity.status(HttpStatus.OK)
				.body(patientRegistrationService.updateHealthIDDetails(healthIdDetailsDTO));
	}

	@GetMapping(path = "/getNdhmFlag")
	public ResponseEntity<NdhmFeatureFlagResponse> getNDHMFeatureFlag() {
		logger.info("Get NDHM feature flag");
		NdhmFeatureFlagResponse response = new NdhmFeatureFlagResponse();
		response.setNdhmFlag(ndhmFeatureFlag);
		logger.info("NDHM flag value::" + ndhmFeatureFlag);
		return ResponseEntity.status(HttpStatus.OK).body(response);
	}

	@PostMapping(path = "/getPatientRefByPatientId", produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<ResponseWrapper<PatientRefResponseDto>> getPatientRefByPatientId(
			@Valid @RequestBody RequestWrapper<PatientRefRequestDto> patientRefRequestDto) {
		logger.info("Receiving healthID Details on telemedicine");
		System.out.println("PatientRefRequestDto " + patientRefRequestDto);
		return ResponseEntity.status(HttpStatus.OK)
				.body(patientRegistrationService.getPatientRefByPatientId(patientRefRequestDto));
	}

	@PostMapping(path = "/getCareContextDetails", produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseWrapper<List<CareContextResponseDto>> getCareContextDetails(
			@Valid @RequestBody RequestWrapper<PatientRefRequestDto> getCareContextByHealthId) {
		logger.info("Receiving healthID Details on telemedicine");
	...
	public ResponseEntity<ResponseWrapper<ReportResponseDto>> getReportsByCareContextIds(@Valid @RequestBody RequestWrapper<ReportRequestDto> reportRequestDtls) {
		logger.info("Receiving CareContextIds Details on telemedicine");
		ResponseWrapper<ReportResponseDto> responseDtls=new ResponseWrapper<ReportResponseDto>();
		try {
			responseDtls.setResponse(patientRegistrationService.getReportsByCareContextIds(reportRequestDtls));
			responseDtls.setStatus(true);	
		} catch (Exception e) {
			logger.error("Error occured while processing request :", e);
			e.printStackTrace();
		}
