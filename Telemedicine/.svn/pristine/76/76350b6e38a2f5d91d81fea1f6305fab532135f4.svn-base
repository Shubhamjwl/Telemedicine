...
@Component({
  selector: 'app-assidte-mobile-otp',
  templateUrl: './assidte-mobile-otp.component.html',
  styleUrls: ['./assidte-mobile-otp.component.scss'],
})
export class AssidteMobileOTPComponent implements OnInit {
  timeLeft: number = 60;
  interval;
  startTimer() {
  ...
  submit() {
    this._servises.generateToken().subscribe(
      ...
      (error) => {
        console.log(error.message, 'Error');
        this.toastrMessage.showErrorMsg('Unable to get token', 'Error');
      }
    );
  }
    ...
    this._servises
      ...
      .subscribe(
        (result) => {
          ...
          this.router.navigate(['/AssistedMobileVerify'], {
            queryParams: {
