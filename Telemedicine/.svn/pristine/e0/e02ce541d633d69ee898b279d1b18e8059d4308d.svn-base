...
import com.nsdl.telemedicine.master.repository.DocumentRepo;
import com.nsdl.telemedicine.master.repository.FeatureCategoryRepo;
...
public class MasterManagementServiceImpl implements MasterManagementService {
	...
	@Autowired
	AssociationNameRepo assocNameRepo;

	@Autowired
	RedFlagUrlRepo redFlagUrlRepo;
	
	@Autowired
	FeatureCategoryRepo featureCategoryRepo;
	...
	@Override
	public List<AssociationNameResponse> getAllAssocNameList() {

		List<AssociationNameResponse> assocNameList = new ArrayList<AssociationNameResponse>();
		List<AssociationNameMasterEntity> allAssociationName = assocNameRepo.findAllAssociationName();
		allAssociationName.forEach(am -> {
			AssociationNameResponse assocName = new AssociationNameResponse();
			assocName.setAssociationName(am.getAssociationName());
			assocName.setId(am.getAssocIdPk());
			assocNameList.add(assocName);
		});
	...
	@Override
	public List<FeatureCategoryResponse> getAllFeaturesCategoryList() {

		List<FeatureCategoryResponse> categoryNameList = new ArrayList<FeatureCategoryResponse>();
		List<FeatureCategoryMasterEntity> allCategoryName = featureCategoryRepo.findAllCategoryName();
		allCategoryName.forEach(fcm -> {
			FeatureCategoryResponse categoryName = new FeatureCategoryResponse();
			categoryName.setId(fcm.getCategoryIdPk());
			categoryName.setCategoryName(fcm.getCategoryName());
	...
	@Override
	public List<DocumentResponse> getAllDocumentList() {
		List<DocumentResponse> docList = new ArrayList<DocumentResponse>();
		List<DocumentMasterEntity> allDocumentName =documentRepo.findAllDocument();
		allDocumentName.forEach(dtm -> {
			DocumentResponse documentName = new DocumentResponse();
			documentName.setId(dtm.getDocIdPk());
			documentName.setDocumentName(dtm.getDocumentName());
			docList.add(documentName);
		});
		return docList;
	}
