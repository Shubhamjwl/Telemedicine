...
public class LinkCallbackController {
	...
	public ResponseEntity<String> discoverInitDummy(@RequestHeader Map<String, String> headers,
			@RequestBody DisLinkingInitReqDTO disLinkingInitReqDTO) throws Exception {
		logger.info("Request Received For Discovery Init Starts {}", disLinkingInitReqDTO);

		/*
		 * if (flag.contentEquals("dummy")) {
		 * linkCallbackService.discoverInitDemo(disLinkingInitReqDTO); /* } else {
...
//		linkCallbackService.discoverInitDemo(disLinkingInitReqDTO);
	...
	public ResponseEntity<String> linkConfirmation(@RequestHeader Map<String, String> headers,
		...
		logger.info("Callback for Link Confirm received {}", linkConfirmDTO);
		/*
		 * if (flag.contentEquals("dummy")) {
		 * linkCallbackService.linkOnConfirm(linkConfirmDTO); /* } else {
		 * linkCallbackService.linkOnConfirmActual(linkConfirmDTO); }
		 */
