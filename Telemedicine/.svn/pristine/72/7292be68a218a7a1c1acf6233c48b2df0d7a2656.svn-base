...
export class PaymentOptionModalComponent implements OnInit {
  ...
  onClickConfirmPayment() {
    if (this.selectedPayOption === 'Continue to payment') {
      ...
      this.dataPassingService.paymentDetails.next({
        ...this.data,
        paitentDetail: this.data.bookApptPayload,
      } as PaymentDetail);
  ...
  onConfirmPayLater() {
    this.spinner.show();
    let payload = {
      transId: this.data.bookApptPayload
        ? this.data.bookApptPayload.transactionID
          ? this.data.bookApptPayload.transactionID
          : null
        : null,
      amount: this.data ? (this.data.fees ? this.data.fees : 0) : 0,
      convenienceCharge: this.data ? (this.data.charge ? this.data.charge : 0) : 0,
      discount: this.data ? (this.data.discount ? this.data.discount : 0) : 0,
      custName: this.data.bookApptPayload
        ? this.data.bookApptPayload.patientName
          ? this.data.bookApptPayload.patientName
          : null
        : null,
      custMobile: this.data.bookApptPayload
        ? this.data.bookApptPayload.patientMNO
          ? this.data.bookApptPayload.patientMNO
          : null
        : null,
      // custMobile: 8446744479, //9604377498
      custEmail: this.data.bookApptPayload
        ? this.data.bookApptPayload.patientEmail
          ? this.data.bookApptPayload.patientEmail
          : null
        : null,
      doctorId: this.data.bookApptPayload
        ? this.data.bookApptPayload.drRegID
          ? this.data.bookApptPayload.drRegID
          : null
        : null,
    };
