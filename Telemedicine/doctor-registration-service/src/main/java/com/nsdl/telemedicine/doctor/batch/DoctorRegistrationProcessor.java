...
public class DoctorRegistrationProcessor implements ItemProcessor<CsvBulkDoctorRequest, DoctorRegDtlsEntity> {
	...
	public DoctorRegDtlsEntity process(CsvBulkDoctorRequest doctor) throws Exception {
		...
		// field validation
		if (!validate.validateFullName(doctor.getFirstName()) || validate.validateFullName(doctor.getLastName()) || !validate.validateMobileNo(doctor.getMobile())
		...
		String userId = utility.createUserId(doctor.getFirstName().toUpperCase());
		if (userId == null) {
			return null;
		}

		DoctorRegDtlsEntity doctorrRegDtlsEntity = new DoctorRegDtlsEntity();
		doctorrRegDtlsEntity.setDrdDrFirstName(doctor.getFirstName().toUpperCase());
		doctorrRegDtlsEntity.setDrdDrMiddleName(doctor.getMiddleName().toUpperCase());
		doctorrRegDtlsEntity.setDrdDrLastName(doctor.getLastName().toUpperCase());
		...
		doctorrRegDtlsEntity.setDrdModifiedBy(doctor.getFirstName().toUpperCase());
