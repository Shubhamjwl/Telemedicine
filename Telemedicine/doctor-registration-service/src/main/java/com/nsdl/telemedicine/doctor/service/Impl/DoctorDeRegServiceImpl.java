		...
		try {
			...
			if (null != userdetails && userdetails.size() > 0) {
				for (LoginUserEntity loginUserEntity : userdetails) {
					...
					if (null != loginUserEntity.getUserFirstName()) {
						if (loginUserEntity.getUserMiddleName() == null && loginUserEntity.getUserLastName() != null) {
							activeDTO.setDocFirstName(
									loginUserEntity.getUserFirstName() + " " + loginUserEntity.getUserLastName());
						} else {
							activeDTO.setDocFirstName(loginUserEntity.getUserFirstName() + " "
									+ loginUserEntity.getUserMiddleName() + " " + loginUserEntity.getUserLastName());
						}
					} else {
						if (doctorMstrDtlsEntity.getDmdDrMiddleName() == null
								&& doctorMstrDtlsEntity.getDmdDrLastName() != null) {
							activeDTO.setDocFirstName(doctorMstrDtlsEntity.getDmdDrFirstName() + " "
									+ doctorMstrDtlsEntity.getDmdDrMiddleName() + " "
									+ doctorMstrDtlsEntity.getDmdDrLastName());
						} else {
							activeDTO.setDocFirstName(doctorMstrDtlsEntity.getDmdDrFirstName() + " "
									+ doctorMstrDtlsEntity.getDmdDrLastName());
						}
