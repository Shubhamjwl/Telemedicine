...
public class MarketPlaceServiceImpl implements MarketPlaceService {
	...
	public MarketPlaceDtlsResponseDTO saveMarketPlaceDetailsUrl(MarketPlaceDetailsDTO marketPlaceDtls) {
		...
		List<MarketPlaceDetailsEntity> checkRecordExist = marketRepo.isRecordExist(marketPlaceDtls.getTranstId());
		if(checkRecordExist!=null&&!checkRecordExist.isEmpty()) {
			marketResponse.setMessage("Prescribe details already exist");
			marketResponse.setClientSecret(marketClientSecret);
			marketResponse.setPatientName(marketPlaceDtls.getPtNname());
			marketResponse.setPatientContact(marketPlaceDtls.getPtMobile());
			return marketResponse;
		}
		marketDtlsEntity.setModTranstIid(marketPlaceDtls.getTranstId());
		marketDtlsEntity.setModDrEmail(marketPlaceDtls.getDrEmail().toUpperCase());
		marketDtlsEntity.setModPtNname(marketPlaceDtls.getPtNname().toUpperCase());
		...
		if (saveDetails != null) {
			marketResponse.setClientSecret(marketClientSecret);
			marketResponse.setPatientName(saveDetails.getModPtNname());
			marketResponse.setPatientContact(saveDetails.getModPtMobile());
			marketResponse.setMessage("Prescribe details saved successfully");
			return marketResponse;
		} else {
	...
	public OrderDetailsResponse saveOrderPlaceDetails(OrderDetailsDTO orderDetailsDTO) {
		...
		marketDtlsEntity.setModDrEmail(orderDetailsDTO.getDrEmail().toUpperCase().trim());
		marketDtlsEntity.setModPtNname(orderDetailsDTO.getPtNname().toUpperCase().trim());
	...
	private OrderDetailsResponse updateOrderDetailsWithStatus(OrderDetailsDTO orderDetailsDTO,
		...
		orderDetailList.forEach(order -> {
			...
			order.setModDrEmail(orderDetailsDTO.getDrEmail().toUpperCase().trim());
			order.setModPtNname(orderDetailsDTO.getPtNname().toUpperCase().trim());
			...
			if (order.getModCreationBy() == null) {
	...
	public List<OrderHistoryDtlsResponse> getOrderHistoryDtls(OrderHistoryDetailsDTO orderHistoryDtls) {
		...
		if (orderHistoryDtls.getPatientName() != null) {
			patientName = orderHistoryDtls.getPatientName().toUpperCase();
		}
		...
		if (patientName != null || orderDate != null || orderId != null) {
			if (userDetails.getRole().equalsIgnoreCase(MarketPlaceConstant.DOCTOR_ROLE)) {
				String drdEmail = docRegRepo.findByUserId(userDetails.getUserName()).getDrdEmail().toUpperCase().trim();
				if (patientName != null && orderId != null && orderDate != null) {
					marketPlaceDetailsList = marketRepo.searchByDoctorFourParameters(drdEmail, orderDate, patientName,
							orderId);
				...
				} else if ((patientName != null && orderDate != null) || (patientName != null && orderId != null)) {
					marketPlaceDetailsList = marketRepo.searchByDoctorThreeParameters(drdEmail, orderDate, patientName,
							orderId);
					if (marketPlaceDetailsList.isEmpty()) {
						logger.error("No search data found");
						throw new MarketPlaceDetailsException(ErrorConstants.NO_SEARCH_DATA_FOUND.getCode(),
					...
					marketPlaceDetailsList = marketRepo.searchByDoctorTwoParameters(drdEmail, orderDate, patientName,
							orderId);
					if (marketPlaceDetailsList.isEmpty()) {
						logger.error("No search data found");
						throw new MarketPlaceDetailsException(ErrorConstants.NO_SEARCH_DATA_FOUND.getCode(),
				...
				String patName = patientPerRepo.findByPatUserId(userDetails.getUserName()).getPtFullName().toUpperCase().trim();
				if ((orderId != null) && (orderDate != null)) {
					...
					marketPlaceDetailsList = marketRepo.searchByPatientOneParameters(orderDate, orderId, patName);
					if (marketPlaceDetailsList.isEmpty()) {
						logger.error("No search data found");
						throw new MarketPlaceDetailsException(ErrorConstants.NO_SEARCH_DATA_FOUND.getCode(),
								ErrorConstants.NO_SEARCH_DATA_FOUND.getMessage());
