...
export class HealthidCreationMobileAssistedComponent implements OnInit {
  ...
  mapHealthIDChecked: boolean = false;
  constructor(
  ...
  verifyFormData() {
    ...
    if (this.form.invalid) {
      ...
      this.toastrMessage.showErrorMsg(ErrorSuccessMessage.PATIENTDETAILS, 'Warning');
      return;
    } else  { 
      if(this.createHealthIDChecked){
        this.spinner.hide();
        this.router.navigate(['/AssistedEnterMobile'], {
          queryParams: { patientName: this.form.get('patientName').value ,mobileNo:this.form.get('mobileNo').value,email:this.form.get('email').value},
        });
  ...
  registerDoctor() {
    ...
    if (this.form.invalid) {
      ...
      this.spinner.hide();
      return;
    } else if(!this.form.get('patientName').value && !this.form.get('patientName').value) {
      this.spinner.hide();
      this.toastrMessage.showErrorMsg(ErrorSuccessMessage.SELECTSMCMCI, 'Warning');
    } else if(this.form.get('mobileNo').value === this.form.get('mobileNo').value) { 
      this.spinner.hide();
