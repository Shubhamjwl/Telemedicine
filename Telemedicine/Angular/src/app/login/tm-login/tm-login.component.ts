...
export class TmLoginComponent implements OnInit {
  ...
  private generateCaptcha() {
    this.authService.generateCaptcha(this.randomStringForCaptcha).subscribe(
      (result) => {
  ...
  verifyCaptcha() {
    if (this.form.invalid) {
      ...
      this.authService.verifyCaptcha(this.randomStringForCaptcha, captcha).subscribe(
        (result) => {
          if (result['status']) {
  ...
  private userSignIn(randomStringForCaptcha, captcha) {
    ...
    this.authService.signIn(data, randomStringForCaptcha, captcha).subscribe(
      (result) => {
        this.spinner.hide();
        if (result && result.response) {
          if (result.response.passwordChanged) {
            this.dialog
              .open(AskToLogoutModalComponent, {
                disableClose: true,
                width: '500px',
                data: {
              ...
              .afterClosed()
              .subscribe({
                next: () => this.router.navigate(['/forgot-password']),
              });
          } else {
            const userID = result?.response?.userId;
            const token = result?.response?.token;
            const role = result?.response?.role;
            this.dataPassingService.loggingUserName = result?.response.userName;
            let closeDrGrp = result?.response?.closeDrGrp || false;
            ...
            if (role == 'PATIENT') {
              ...
              this.router.navigate(['../admin'], { relativeTo: this.activateRoute });
          ...
          if (result.loggedInFlag) {
            ...
            this.refreshCpacha();
            const msg = result.errors[0]?.message?.replace(/\n/g, '<br />') || 'Error';
            this.toastMessage.showInfoMsg(msg, 'Information', { timeOut: 5000, enableHtml: true });
          }
          this.spinner.hide();
        }
      },
  ...
  private userSignOut(userID) {
    this.authService.signOutWithOutToken(userID).subscribe(
      (result) => {
  ...
  private getOtp() {
    ...
    this.authService.getUserDetailsAndSendOTP(userId, fromWhere).subscribe(
      (result: any) => {
        this.spinner.hide();
        if (result.status && result.response) {
          this.toastMessage.showSuccessMsg(result.response.description, 'Success');
          this.router.navigate(['/forgot-password'], { queryParams: { fromWhere: 'forgot' } });
