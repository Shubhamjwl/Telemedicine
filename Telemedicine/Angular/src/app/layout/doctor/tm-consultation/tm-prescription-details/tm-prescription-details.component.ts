...
@Component({
  selector: 'app-tm-prescription-details',
  templateUrl: './tm-prescription-details.component.html',
  styleUrls: ['./tm-prescription-details.component.scss'],
})
export class TmPrescriptionDetailsComponent implements OnInit, OnDestroy {
  ...
  medicineTypes = ['Cap.', 'Syp.', 'Inj.', 'Resp.', 'Sach.', 'Oint.'];
  medicineDoses = [1, 2, 5, 10];
  medicineFrequency = ['Morning', 'Afternoon', 'Evening', 'Night'];
  medicineMeal = ['Before Meal', 'After Meal', 'Empty Stomach', 'As Instructed'];
  medicineDuration = [1, 2, 5, 10, 'SOS'];
  constructor(
  ...
  getPrescriptionDetails() {
    this.consultationService.getSymptomsDetails().subscribe((result) => {
      ...
      let selectedSymptoms = this.consultationService.patientConsulation.selectedSymptoms;
      if (selectedSymptoms) {
      ...
      this.form.controls.symptomName.valueChanges.pipe(takeUntil(this._onDestroy)).subscribe(() => {
    ...
    this.consultationService.getAdviceDetails().subscribe((result) => {
      ...
      let selectedAdvice = this.consultationService.patientConsulation.selectedAdvice;
      if (selectedAdvice) {
      ...
      // listen for search field value changes
      this.form.controls.adviceName.valueChanges.pipe(takeUntil(this._onDestroy)).subscribe(() => {
    ...
    this.consultationService.getDiagnosisDetails().subscribe((result) => {
      ...
      let selectedDiagnosis = this.consultationService.patientConsulation.selectedDiagnosis;
      ...
      // listen for search field value changes
      this.form.controls.diagnosisName.valueChanges.pipe(takeUntil(this._onDestroy)).subscribe(() => {
    ...
    this.consultationService.getMedicinesDetails().subscribe((result) => {
      ...
      let selectedMedicines = this.consultationService.patientConsulation.selectedMedicines;
      ...
      // listen for search field value changes
      this.form.controls.medicineName.valueChanges.pipe(takeUntil(this._onDestroy)).subscribe(() => {
  ...
  protected filterDetails(details: any, searchValue: string, subject: BehaviorSubject<any>) {
    ...
    // filter the details
    subject.next(details.filter((obj) => obj.name.toLowerCase().indexOf(searchValue.toLowerCase()) >= 0));
    this.sort(subject);
  }
  ...
  saveMedicine() {
    if (this.form.controls.medicineName.value) {
      let entry = this.medicineDetails.value.find((el) => el.name === this.form.controls.medicineName.value);
  ...
  saveDetails(name: any, details: FormControl) {
    if (name.value) {
      let entry = details.value.find((el) => el.name === name.value);
  ...
  savePrescriptionDetails() {
    ...
    let request = {
      ...
      ctChiefComplaints: this.symptomDetails.value?.length
        ? Array.prototype.map
            .call(this.symptomDetails.value, function (item) {
              return item.name;
            })
            .join(', ')
        : null,
      ctDiagnosis: this.diagnosisDetails.value?.length
        ? Array.prototype.map
            .call(this.diagnosisDetails.value, function (item) {
              return item.name;
            })
      ...
      ctAdvice: this.adviceDetails.value?.length
        ? Array.prototype.map
            .call(this.adviceDetails.value, function (item) {
              return item.name;
            })
            .join(', ')
        : null,
    ...
    this.consultationService.patientConsulation.selectedSymptoms = this.symptomDetails.value;
    this.consultationService.patientConsulation.selectedDiagnosis = this.diagnosisDetails.value;
    this.consultationService.patientConsulation.selectedMedicines = this.medicineDetails.value;
    this.consultationService.patientConsulation.selectedAdvice = this.adviceDetails.value;

    if (
      !this.symptomDetails.value?.length &&
      !this.diagnosisDetails.value?.length &&
      !this.medicineDetails.value?.length &&
      !this.adviceDetails.value?.length &&
      !this.profilePhotoBase64
    ) {
      if (this.stepper.selectedIndex === 1) {
        this.toastrMessage.showWarningMsg('Please enter Prescription details', 'Warning');
        this.prescriptionDataSharingService.prescriptionDataFilled.next(false);
      }
    } else {
      this.consultationService.savePrescriptionDetails(request).subscribe({
        next: (res) => {
          if (res.status) {
            this.prescriptionDataSharingService.prescriptionDataFilled.next(true);
          } else {
            this.toastrMessage.showSuccessMsg(res?.response?.message, 'Error');
          }
        },
      });
    }
  ...
  buildMedicineContent(): string {
    ...
    this.medicineDetails.value.forEach((medicine) => {
      ...
      if (medicine.period !== undefined) {
        period = ' x ' + '(' + medicine.period + ' day/s)';
      }
      let meal = '';
  ...
  typeValChange(medicineDetails: any, input: string) {
    medicineDetails.type = input;
    this.changeEvent();
  }

  frequencyValChange(medicineDetails: any, input: string) {
    medicineDetails.frequency = medicineDetails.frequency || [];
    if (!medicineDetails.frequency.includes(input)) {
      medicineDetails.frequency.push(input);
    } else {
      medicineDetails.frequency.splice(medicineDetails.frequency.indexOf(input), 1);
    }
    this.changeEvent();
  }

  mealValChange(medicineDetails: any, input: string) {
    medicineDetails.meal = input;
    this.changeEvent();
  }
  ...
  includesCheck(data: any, input: string) {
    if (data !== undefined && data.includes(input)) {
      return 1;
    } else {
      return 0;
    }
  ...
  onDoseToggleValChange(medicine: any, value: string) {
    medicine.units = value;
    this.changeEvent();
  }

  onDurationToggleValChange(medicine: any, value: string) {
    medicine.period = value;
    this.changeEvent();
  }
}
