...
export class ProfileCreateComponent implements OnInit {
  ...
  staticCountryCode: any = '+91';
  selectOptions = ['Male', 'Female', 'Transgender'];

  

  selectedOption = '';
  private healthIdInput$ : Subject<string> = new Subject();
  ...
  onHealthIdChange(value: string) {
    this.healthIdInput$.next(value);
  }

  private healthIdValidation() {
    this.healthIdInput$.pipe(debounceTime(500), distinctUntilChanged(),
      switchMap((value) => this.validateHealthId(value))).subscribe(res => {
        if (res.errors?.length) {
          this.healthIdError = res.errors[0]?.message;
        } else {
          this.healthIdError = '';
        }
      })
  ...
  private validateHealthId(healthId: string): Observable<MainResponseDTO<any>> {
    if (healthId) {
      return this._servises.validateHealthId(healthId, this.mobileNoSel).pipe(
        switchMap(res => {
          if (res.status) {
            return of(res)
          } else {
            return this._servises.searchByHealthId('', { healthId })
          }
        })
      )
    } else {
     return of({} as MainResponseDTO<any>);
    }
  }
}
