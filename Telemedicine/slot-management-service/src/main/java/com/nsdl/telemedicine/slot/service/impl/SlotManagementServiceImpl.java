...
import com.nsdl.telemedicine.slot.exception.SlotServiceException;
import com.nsdl.telemedicine.slot.exception.ServiceErrors;
...
public class SlotManagementServiceImpl extends SlotConstant implements SlotManagementService {
	...
	/*	List<DocSlotDtlsEntity> slotlist=new ArrayList<>();
		...
		try {
			...
			/*if(req.getSlotType().length()<=0) {
				errorList=SlotUtility.getExceptionList(errorList, ErrorConstants.INVALID_SLOT_TYPE.getCode(), ErrorConstants.INVALID_SLOT_TYPE.getMessage());
				logger.error(ErrorConstants.INVALID_SLOT_TYPE.getMessage());				
			}*/
			...
			if(daysTime.size()>0) {
				...
				DrSlotMstr save = slotMstrRepository.save(drSlotMstr);
...
private DrSlotMstr getDrSlotMstr(DrSlotMstr drSlotMstr,String days, String completeTime) {
	...
	public MainResponseDTO<String> holidayManagement(MainRequestDTO<HolidayReqtDto> masterRequest) {
		...
		String holidayMark="System is able to mark holiday on ";
		String holidayNotMark="Not able to mark holiday on ";
		...
		if(errorList!=null) {
			...
			if(mark && notMark)
				holidayMark=holidayMark+" "+holidayNotMark+" Because appointments are present on that day. ";
			else if(notMark && !mark)
				holidayMark=holidayNotMark+" Because appointments are present on that day. ";
	...
	@Override
	public String deleteSlot(DeleteSlotRequestDTO masterRequest) {

		Date slotDate = masterRequest.getSlotDate();
		if (slotDate.before(getDateWithZeroTime(new Date()))) {
			logger.error("Can't delete past date slots");
			throw new SlotServiceException(new ServiceErrors(ErrorConstants.CANT_DELETE_PAST_DATE_SLOTS.getCode(),
					ErrorConstants.CANT_DELETE_PAST_DATE_SLOTS.getMessage()));
		}

		for (String slot : masterRequest.getSlotTime()) {
			if (!validater.validateSlotTime(slot)) {
				logger.error(ErrorConstants.INVALID_SLOT_TIME.getMessage());
				throw new SlotServiceException(new ServiceErrors(ErrorConstants.INVALID_SLOT_TIME.getCode(),
						ErrorConstants.INVALID_SLOT_TIME.getMessage() + slot));
			}
		}

		String userId = userDetails.getRole().equalsIgnoreCase(DOCTOR_ROLE) ? userDetails.getUserName() : null;

		if (userId == null) {
			throw new SlotServiceException(new ServiceErrors(ErrorConstants.INVALID_DOC_DETAILS.getCode(),
					ErrorConstants.INVALID_DOC_DETAILS.getMessage()));
		}

		List<AppointmentDtlsEntity> appointments = appointmentDtlsRepository.getSlotsByDateTime(userId,
				masterRequest.getSlotTime(), slotDate);

		if (appointments != null && !appointments.isEmpty()) {
			throw new SlotServiceException(new ServiceErrors(ErrorConstants.CANT_DELETE_APPOINTED_SLOTS.getCode(),
					ErrorConstants.CANT_DELETE_APPOINTED_SLOTS.getMessage()));
		}

		List<DocSlotDtlsEntity> entities = masterRepo.findSlotsByDateTime(userId, masterRequest.getSlotTime(),
				slotDate);

		// saving deleted slots into audit
		if (entities != null && !entities.isEmpty()) {
			List<AudDocSlotDtlsEntity> auditEntities = new ArrayList<>();
			for (DocSlotDtlsEntity entity : entities) {
				AudDocSlotDtlsEntity audDocSlotDtlsEntity = new AudDocSlotDtlsEntity(entity, userId);
				auditEntities.add(audDocSlotDtlsEntity);
			}
			autidRepo.saveAll(auditEntities);
			masterRepo.deleteAll(entities);
		}

		if (entities == null || entities.size() == 0) {
			throw new SlotServiceException(new ServiceErrors(ErrorConstants.NO_SLOT_DETAILS_FOUND.getCode(),
					ErrorConstants.NO_SLOT_DETAILS_FOUND.getMessage()));
		} else if (entities.size() == masterRequest.getSlotTime().size()) {
			return SLOT_DELETED;
		} else {
			return PARTIALLY_SLOT_DELETED + " count: " + entities.size();
		}
	}

	private static Date getDateWithZeroTime(Date date) {
		Calendar calendar = Calendar.getInstance();
		calendar.setTime(date);
