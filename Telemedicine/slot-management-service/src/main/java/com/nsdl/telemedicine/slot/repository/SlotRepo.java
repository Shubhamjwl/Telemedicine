...
public interface SlotRepo extends JpaRepository<DocSlotDtlsEntity, String>{
	...
	@Query(value = " SELECT * FROM appointment.dr_slot_dtls s WHERE s.dsd_dr_user_id_fk = :userName " + 
			" and s.dsd_isactive = true and EXTRACT(YEAR from s.dsd_slot_date)=:year  " + 
			" and EXTRACT(MONTH from s.dsd_slot_date)=:month and NOT EXISTS ( select 1 from appointment.appt_dtls a where " + 
			" s.dsd_dr_user_id_fk = a.ad_dr_user_id_fk and s.dsd_slot = a.ad_appt_slot_fk and " + 
			" s.dsd_Slot_date = a.ad_appt_date_fk and a.ad_appt_status in ('C','S','R')) " + 
			" order by s.dsd_Slot_date asc;", nativeQuery = true)
	public List<DocSlotDtlsEntity> getHistoricalSlotDetailsByMonth(String userName, int year, int month);
	
	@Query(value = "SELECT * FROM appointment.dr_slot_dtls where dsd_dr_user_id_fk =:dsd_dr_user_id_fk and dsd_isactive ='true'  ORDER BY dsd_slot_date ;", nativeQuery = true)
	public List<DocSlotDtlsEntity> getSlotDetailsForDoctor(@Param("dsd_dr_user_id_fk") String dsd_dr_user_id_fk);
	
	@Query(value = "SELECT * FROM appointment.dr_slot_dtls where dsd_dr_user_id_fk = ?1 and dsd_slot in ?2 and dsd_slot_date = ?3", nativeQuery = true)
	public List<DocSlotDtlsEntity> findSlotsByDateTime(String drUserId, List<String> time, Date date);
