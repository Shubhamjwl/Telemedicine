...
public class Utility {
	...
	public static String getTemplateTypeName(String tempType) {
		...
		} else if (tempType.equalsIgnoreCase(AppConstant.PAYMENT_SUCCESS)) {
			templateName = TemplateType.PAYMENT_SUCCESS.templateTypeName();

		} else if (tempType.equalsIgnoreCase(AppConstant.PAYMENT_FAIL)) {
			templateName = TemplateType.PAYMENT_FAIL.templateTypeName();

		}
		//
		else if (tempType.equalsIgnoreCase(AppConstant.DOC_DEREGISTER)) {
			templateName = TemplateType.DOC_DEREGISTER.templateTypeName();

		} else if (tempType.equalsIgnoreCase(AppConstant.DE_REG)) {
			templateName = TemplateType.DEREG.templateTypeName();

		} else if (tempType.equalsIgnoreCase(AppConstant.PATIENT_REGISTER)) {
			templateName = TemplateType.PATIENT_REGISTER.templateTypeName();

		} else if (tempType.equalsIgnoreCase(AppConstant.CONSULTATION_DOWNLOAD_PRESCRIPTION_PT)) {
			templateName = TemplateType.CONSULTATION_PRESCRIPTION_PT.templateTypeName();

		} else if (tempType.equalsIgnoreCase(AppConstant.LINK_RECORD)) {
			templateName = TemplateType.LINK_RECORD.templateTypeName();
